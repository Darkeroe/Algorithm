    ...
    for i in copy_order:
        if i[0] != -1:
            ...
            for j in range(-1,7):
                nx = i[0] + dx[tmp[1]+j]
                ny = i[1] + dy[tmp[1]+j]
                if 0 <= nx < 4 and 0 <= ny < 4 and copy_bowl[nx][ny][0] != 0: # 0: 상어, -1: 공란
                    if copy_bowl[nx][ny][0] == -1:
                        copy_order[tmp[0]-1] = [nx,ny]
                    else:
                        copy_order[tmp[0]-1], copy_order[copy_bowl[nx][ny][0]-1] = [nx,ny], i
                    copy_bowl[i[0]][i[1]][1] = (tmp[1]+j+1)%8
                    copy_bowl[i[0]][i[1]], copy_bowl[nx][ny] = copy_bowl[nx][ny], copy_bowl[i[0]][i[1]]
                    break
    return copy_order
...
for i in range(4):
        bowl[i][0][0], bowl[i][0][1], bowl[i][1][0], bowl[i][1][1], bowl[i][2][0], bowl[i][2][1], bowl[i][3][0], bowl[i][3][1] = map(int,input().split())
